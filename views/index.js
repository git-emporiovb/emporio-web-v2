    ...
    try {
        ...
        for (var l = 0; l < results.length; l++) {
            ...
            var item = {
                ...
                image: results[l].get("image").url().replace("http://localhost", "https://emporio-server-v2-a37450f98aea.herokuapp.com"),
            ...
            categories_cards += `
              <div style="width:100px;padding: 0 15px">
                <div class="card d-flex justify-content-center align-items-center">
                  <!-- Image -->
                  <div class="image"><img src="${categories[m].get("image")?.url().replace("http://localhost", "https://emporio-server-v2-a37450f98aea.herokuapp.com")}"></div>
                  <!-- Body -->
        ...
        for (var i = 0; i < banners.length; i++) {
            ...
            slides +=
                ...
                banners[i].get("image_desktop").url().replace("http://localhost", "https://emporio-server-v2-a37450f98aea.herokuapp.com") +
                '"><img class="mobile" src="' +
                banners[i].get("image_mobile").url().replace("http://localhost", "https://emporio-server-v2-a37450f98aea.herokuapp.com") +
                '"></a>';
        }
      ...
      query.equalTo('tertiary_category',decodeURI(req.query.tertiary_category));
        ...
        for (var i = 0; i < results.length; i++) {
            ...
            var item = {
                ...
                image: results[i].get("image").url().replace("http://localhost", "https://emporio-server-v2-a37450f98aea.herokuapp.com")
            };
...
router.get("/items/*", async function (req, res) {
    ...
    try {
        ...
        var item = {
            id: object.id,
            image: object.get("image").url().replace("http://localhost", "https://emporio-server-v2-a37450f98aea.herokuapp.com"),
                ...
                if (Number(req.body[i].quantity) <= item.get("stock")) {
                    ...
                    items.push({
                        item: {
                            ...
                            image: item.get("image").url().replace("http://localhost", "https://emporio-server-v2-a37450f98aea.herokuapp.com"),
        ...
        for (var j = 0; j < order.items.length; j++) {
            ...
            if (order.items[j].get("item").get("image")) {
                images +=
                    ...
                    order.items[j].get("item").get("image").url().replace("http://localhost", "https://emporio-server-v2-a37450f98aea.herokuapp.com") +
    ...
    try {
        ...
        for (var i = 0; i < order.items.length; i++) {
            ...
            rows +=
                ...
                item.id +
                '"><img src="' +
                item.get("image").url().replace("http://localhost", "https://emporio-server-v2-a37450f98aea.herokuapp.com") +
                '" alt="' +
...
async function parseUser(pointer) {
    ...
    if (pointer.get("image")) {
        user.image = pointer.get("image").url().replace("http://localhost", "https://emporio-server-v2-a37450f98aea.herokuapp.com").replace("http://localhost", "https://emporio-server-v2-a37450f98aea.herokuapp.com");
    } else {
